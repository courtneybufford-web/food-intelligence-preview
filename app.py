...
def serving_size_to_grams(value, unit):
    """Convert serving-size UI values to grams when the unit is weight-based.

    This is intentionally conservative. Volume/count units like cup, tbsp, each,
    serving, or portion need item-specific density/yield data, so they are used
    as label text only unless the user chooses Custom serving weight (g).
    """
    ...
    if unit in ["g", "kg", "mg", "oz", "lb"]:
        return value * UNIT_TO_GRAMS.get(unit, 0.0)
    return 0.0
...
def calculate_label_nutrition(total, servings, total_weight_g, serving_option, serving_size_value=1.0, serving_size_unit="serving", custom_serving_weight_g=0.0):
    ...
    if serving_option == "Per 100 g":
        ...
        return scale_total_nutrition(total, factor), "100 g", (round(total_weight_g / 100.0, 1) if total_weight_g > 0 else servings)
...
with tabs[4]:
    ...
    if not st.session_state.recipe_items:
        ...
        if serving_option == "Per serving (FDA default)" and serving_size_unit not in ["g", "kg", "mg", "oz", "lb"]:
            st.caption("Serving size units like cup, each, slice, serving, and portion are used as label text only unless you enter a custom serving weight in grams. Weight units update nutrition values directly.")
